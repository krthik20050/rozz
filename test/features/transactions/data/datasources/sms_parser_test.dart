...
void main() {
  ...
  test('credit SMS extracts the VPA sender, not the own account', () {
    final result = parser.parse(
      'Credit Alert!\nRs.200.00 credited to HDFC Bank A/c XX4321 on 06-08-26 from VPA 919812345678@wahdfcbank (UPI 127493362828)',
    );
    expect(result, isNotNull);
    expect(result!['recipient_name'], '919812345678@wahdfcbank');
    expect(result['direction'], 'credit');
  });

  test('credit SMS keeps the VPA local part intact (tfam9396@ybl)', () {
    final result = parser.parse(
      'Credit Alert!\nRs.1000.00 credited to HDFC Bank A/c XX4321 on 03-08-26 from VPA tfam9396@ybl (UPI 860638380533)',
    );
    expect(result!['recipient_name'], 'tfam9396@ybl');
  });

  test('debit with no named payee leaves recipient null', () {
    final result = parser.parse(
      'Rs.24.00 debited from HDFC Bank A/c XX4321 on 14-08-26. Avl Bal Rs.59.67. Not You? Call 18002586161.',
    );
    expect(result, isNotNull);
  ...
  test('daily balance advice is a balance snapshot, not a transaction', () {
    final result = parser.parse(
      'Available Bal in HDFC Bank A/c XX4321 as on yesterday:14-AUG-26 is INR 59.67. '
  ...
  test('standard balance advice (no "yesterday") is a snapshot', () {
    final result = parser.parse(
      'Available Bal in A/c XX4321 is INR 59.67 as on 14-AUG-26. '
      'Cheques are subject to clearing.For updated A/C Bal dial 18002703333.',
    );
    expect(result, isNotNull);
    expect(result!['label_type'], 'balance_snapshot');
