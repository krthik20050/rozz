...
void main() {
  ...
  final monthTxns = [
    ...
    TransactionModel(
      ...
      direction: 'credit',
      labelType: 'upi',
      recipientName: 'TT9396@OKHDFCBANK',
      source: 'sms',
    ),
  ];
  ...
  blocTest<InsightsBloc, InsightsState>(
    ...
    build: () {
      ...
      when(() => mockLabelRepository.getAll()).thenAnswer((_) async => [
            const SenderLabel(key: 'tt9396', label: 'father'),
          ]);
      return bloc;
    },
  ...
  blocTest<InsightsBloc, InsightsState>(
    ...
    act: (bloc) => bloc.add(SaveSenderLabel(key: 'tt9396', label: 'father')),
    expect: () => [
      isA<InsightsLoading>(),
      isA<InsightsLoaded>(),
    ],
  ...
  blocTest<InsightsBloc, InsightsState>(
    ...
    act: (bloc) => bloc.add(DeleteSenderLabel(key: 'tt9396')),
    expect: () => [
      isA<InsightsLoading>(),
      isA<InsightsLoaded>(),
    ],
    verify: (_) {
      verify(() => mockLabelRepository.delete('tt9396')).called(1);
    },
  );
